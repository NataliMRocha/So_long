...
/*   Updated: 2023/10/24 18:16:50 by namoreir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_matrix(t_def **def, const char *argv)
{
	...
	string[i] = '\0';
	(*def)->map->map = ft_split(string, '\n');
	if ((*def)->map->map == NULL)
		ft_free_validation(def, 0, 1);
	(*def)->map->copy = ft_split(string, '\n');
